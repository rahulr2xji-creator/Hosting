...
# --- Configuration ---
TOKEN = '8624297353:AAHohGHbPAphrACcFyoSANNpeImxQopY6-I'
OWNER_ID = 7898402627
ADMIN_ID = 7898402627
YOUR_USERNAME = '@SEMY0HERE'
UPDATE_CHANNEL = 'https://t.me/SEMY_FF'
# Folder setup
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🚀 <b>𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇: STARTING SCRIPT</b> 🚀 ║
╠══════════════════════════════════════╣
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🟢 <b>𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇: STARTING NODE.JS</b> 🟢║
╠══════════════════════════════════════╣
║ 📄 File: <code>{file_name[:25]}</code>
║ 👤 User: {script_owner_id}
║ 🔄 Attempt: {attempt}/{max_attempts}
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇: NODE.JS RUNNING</b> ✅  ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ 🆔 <b>PID:</b> {process.pid}
║ ⏱️ <b>Started:</b> {datetime.now().strftime('%H:%M:%S')}
...
def stats_command(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐆𝐚𝐭𝐡𝐞𝐫𝐢𝐧𝐠 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 𝐬𝐭𝐚𝐭𝐬...", duration=2)
...
def speed_command(message):
    ...
    msg = send_spinner_animation(message.chat.id, "𝐓𝐞𝐬𝐭𝐢𝐧𝐠 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 𝐬𝐩𝐞𝐞𝐝...", duration=2)
...
"""
    ...
    result_text = f"""
╔══════════════════════════════════════╗
║     ✅ <b> 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄</b> ✅ ║
╠══════════════════════════════════════╣
║
║  📤 𝐓𝐨𝐭𝐚𝐥: {total}
║  ✅ 𝐒𝐮𝐜𝐜𝐞𝐬𝐬: {success}
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel":
        bot.send_message(message.chat.id, f"📢 𝐉𝐨𝐢𝐧 𝐨𝐮𝐫 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 𝐮𝐩𝐝𝐚𝐭𝐞𝐬:\n{UPDATE_CHANNEL}")
...
def show_user_files(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐋𝐨𝐚𝐝𝐢𝐧𝐠 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 𝐟𝐢𝐥𝐞𝐬...", duration=1)
    files = user_files.get(user_id, [])
    if not files:
        text = """
╔══════════════════════════════════════╗
║       📂 <b>𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇: YOUR FILES</b> 📂   ║
╠══════════════════════════════════════╣
║
...
def show_admin_logs(call):
    ...
    try:
        ...
        conn.close()
        if logs:
            text = "📋 <b>𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇: RECENT LOGS</b>\n"
            for log in logs:
                text += f"👤 {log[0]} | {log[1]}\n{log[2][:30]}...\n🕐 {log[3][:16]}\n"
        else:
            text = "📋 𝐍𝐨 𝐥𝐨𝐠𝐬."
...
def cleanup_on_exit():
    ...
    logger.info("𝐂𝐥𝐞𝐚𝐧𝐢𝐧𝐠 𝐮𝐩 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇...")
    for script_key in list(bot_scripts.keys()):
        try:
            script_info = bot_scripts[script_key]
            kill_process_tree(script_info)
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 🦁 𝐁𝐨𝐭...")
    ...
    while True:
        try:
            ...
            logger.error(f"𝙉4𝙔3𝙈 𝙊𝙁𝘾𝙇 𝐞𝐫𝐫𝐨𝐫: {e}", exc_info=True)
            time.sleep(5)
